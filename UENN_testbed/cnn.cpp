...
CNNMNIST::CNNMNIST(Options& options) :
    m_options(options),    
 ...
 */
void CNNMNIST::SetOptimizer(unsigned long optimizer)
{
    switch (optimizer)
    {
...
void CNNMNIST::loadMNIST(const char* kDataRoot)
{
...
void CNNMNIST::trainCNN(size_t epoch, DataLoader& data_loader, unsigned long lossfunctype)
{
 ...
 */
template <typename DataLoader>
void CNNMNIST::trainENN(size_t epoch, DataLoader& data_loader,
 ...
 *
 */
void CNNMNIST::printENNTrainingData(size_t &epoch, size_t batch_size, 
 ...
 * 
 */
void CNNMNIST::testCNN(unsigned long lossfunctype)
{
    // 0.1307, 0.3081 are the mean and std deviation of the MNIST dataset.
 ...
 */
template <typename DataLoader>
void CNNMNIST::testCNN(DataLoader& data_loader, unsigned long lossfunctype)
{
 ...
 */
template <typename DataLoader>
void CNNMNIST::testENN(DataLoader& data_loader, 
    torch::Tensor(*lossfunc)(const torch::Tensor&, const torch::Tensor&,
        size_t, size_t, size_t, torch::Device))
{
    torch::NoGradGuard no_grad;
