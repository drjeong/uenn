...
// Function to print tensor to cout
inline void printTensorSize(const torch::Tensor& tensor, std::string label)
{
	std::cout << std::endl;
...
inline void printTensorType(const torch::Tensor& tensor, std::string label)
{
...
inline bool outputToFileAndConsole(const std::string& filename, const std::string& msg) {
...
inline std::vector<float> flattenTensor(const torch::Tensor& tensor) {
...
/// <returns></returns>
inline std::vector<std::vector<float>> tensorToVector(const torch::Tensor& tensor)
{
	// Convert tensor to CPU and to a float tensor if not already
	torch::Tensor cpuTensor = tensor.cpu();
	torch::Tensor floatTensor = cpuTensor.to(torch::kFloat32);
